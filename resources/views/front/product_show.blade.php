...
@section('content')
    ...
    <style>
        ...
        .thumbs .thumb-box:hover,
        .thumbs .thumb-box.active {
            border-color: #843c24;
        }
            ...
            <div class="row">
                        ...
                        <div class="thumbs mt-3 d-flex justify-content-center gap-2 flex-wrap">
                            @foreach ($product->gallery_images as $index => $thumb)
                                <div class="thumb-box border rounded overflow-hidden {{ $index === 0 ? 'active' : '' }}"
                                    data-image="{{ asset($thumb) }}" onclick="changeMainImage(this)">
                                    <img src="{{ asset($thumb) }}" alt="Thumbnail {{ $index + 1 }}"
                                        class="img-thumbnail" />
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="col-md-7">
                    ...
                    <h1 class="product-title">
                        {{ app()->getLocale() === 'ar' ? $product->product_name_ar : $product->product_name_en }}</h1>
                    ...
                    <p class="product-status mb-2">
                        ...
                        <span
                            class="badge bg-{{ $product->status === 'active' ? 'success' : ($product->status === 'inactive' ? 'danger' : 'warning') }}">
                    ...
                    <p class="product-quantity mb-3">
                        ...
                        <span
                            class="{{ $product->quantity < 1 ? 'text-danger' : 'text-success' }}">{{ $product->quantity }}
                            {{ __('items') }}</span>
                    </p>
                    ...
                    <div class="description mb-4">
                        <p class="product-desc">
                            {!! $product->{'description_' . app()->getLocale()} ?: __('No description available.') !!}</p>
                    </div>
                    ...
                    <form method="POST" class="add-to-cart-form">
                        ...
                        <div class="input-group mb-3" style="max-width: 150px;">
                            ...
                            <input type="number" name="quantity" class="form-control text-center qty-input" value="1"
                                min="1" max="{{ $product->quantity }}" readonly aria-label="{{ __('Quantity') }}">
                    ...
                    <div class="share-section mt-4">
                        ...
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}" target="_blank"
                            class="me-2" aria-label="{{ __('Share on Facebook') }}"><i class="fab fa-facebook"></i></a>
                        <a href="https://twitter.com/intent/tweet?url={{ url()->current() }}&text={{ app()->getLocale() === 'ar' ? $product->product_name_ar : $product->product_name_en }}"
                            target="_blank" class="me-2" aria-label="{{ __('Share on Twitter') }}"><i
                                class="fab fa-twitter"></i></a>
                        <a href="https://wa.me/?text={{ url()->current() }}" target="_blank"
                            aria-label="{{ __('Share on WhatsApp') }}"><i class="fab fa-whatsapp"></i></a>
                    </div>
                </div>
                        ...
                        @foreach ($relatedProducts as $related)
                            <div class="col-md-3 mb-4">
                                <div class="card h-100 shadow-sm">
                                    <img src="{{ $related->main_image_url }}" class="card-img-top"
                                        alt="{{ app()->getLocale() === 'ar' ? $related->product_name_ar : $related->product_name_en }}">
                                    <div class="card-body">
                                        <h5 class="card-title">
                                            {{ Str::limit(app()->getLocale() === 'ar' ? $related->product_name_ar : $related->product_name_en, 30) }}
                                        </h5>

    ...
    <div class="row">
                        ...
                        <article class="product-card w-100">
                            <a href="{{ route('front.product-details', ['id' => $product->id]) }}" style="text-decoration: none; color: inherit;">
                                ...
                                <div class="product-card-content px-3">
                                    <h6 class="product-title">{{ app()->getLocale() === 'ar' ? $product->product_name_ar : $product->product_name_en }}</h6>
        ...
        <div class="col-lg-3 sidebar">
            ...
            <ul>
                @foreach ($categories as $category)
                    <li class="{{ request()->query('category') == $category->id ? 'active' : '' }}">
                        <a href="{{ route('front.product', ['category' => $category->id]) }}">{{ app()->getLocale() === 'ar' ? $category->name_ar : $category->name_en }}</a>
                    </li>
                @endforeach
            </ul>
            ...
            <ul>
                @foreach ($subcategories as $subcategory)
                    <li class="{{ request()->query('subcategory') == $subcategory->id ? 'active' : '' }}">
                        <a href="{{ route('front.product', ['subcategory' => $subcategory->id]) }}">{{ app()->getLocale() === 'ar' ? $subcategory->name_ar : $subcategory->name_en }}</a>
                    </li>
                @endforeach
            </ul>
            ...
            <form action="{{ route('front.product') }}" method="GET" class="search-box input-group">
                <input type="text" name="search" class="form-control" placeholder="{{ __('Search') }}..." value="{{ request()->query('search') }}">
            ...
            @foreach ($bestSellers as $bestSeller)
                <div class="best-seller">
                    ...
                    <span>{{ app()->getLocale() === 'ar' ? $bestSeller->product_name_ar : $bestSeller->product_name_en ?? 'Unnamed Product' }}<br>{{ $bestSeller->display_price_formatted }} {{ $currency }}</span>
                </div>
            @endforeach
        </div>
    </div>
